...
trace m f exitIfSeen seen g =
  let seen' = Set.insert (f g) seen
      (Guard d p) = g
   in case Map.lookup (d `from` p) m of
        Nothing -> (seen', False)
        Just False -> trace m f exitIfSeen seen' (Guard d (d `from` p))
        Just True -> trace m f exitIfSeen seen' (Guard (clockwise d) p)

countLoops :: Lab -> Set Pos -> Guard -> Int
countLoops m tried g = loopsFromHere + loopsFromRest
 where
  inFrontOf (Guard d p) = d `from` p
  addInFront :: Lab
  addInFront = Map.update (const $ Just True) (inFrontOf g) m

  tried' = Set.insert (inFrontOf g) tried

  loopsFromHere = if inFrontOf g `Set.notMember` tried && endsInLoop addInFront g then 1 else 0
  loopsFromRest = sum $ countLoops m tried' <$> step m g
